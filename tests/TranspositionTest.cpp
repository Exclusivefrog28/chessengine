...
TEST_CASE("TT - rw", "[TranspositionTests]") {
	...
	tt.setEntry(board, parseMove("f6e7", board), 1, 1, TranspositionTable::EXACT, 1);
	const auto readEntry = tt.getEntry(board.hashCode, 1);
	CHECK(readEntry.bestMove == parseMove("f6e7", board));
...
TEST_CASE("TT - overwrite", "[TranspositionTests]") {
	...
	tt.setEntry(board, parseMove("h4b4", board), 2, 1, TranspositionTable::LOWERBOUND, 1);
	auto readEntry = tt.getEntry(board.hashCode, 1);
	CHECK(readEntry.score == 1);

	tt.setEntry(board, parseMove("h4b4", board), 1, 2, TranspositionTable::LOWERBOUND, 1);
	readEntry = tt.getEntry(board.hashCode, 1);
	CHECK(readEntry.score == 1);

	tt.setEntry(board, parseMove("h4b4", board), 3, 3, TranspositionTable::LOWERBOUND, 1);
	readEntry = tt.getEntry(board.hashCode, 1);
	CHECK(readEntry.score == 3);

	tt.setEntry(board, parseMove("h4b4", board), 2, 4, TranspositionTable::EXACT, 1);
	readEntry = tt.getEntry(board.hashCode, 1);
...
TEST_CASE("TT - mate recall", "[TranspositionTests]") {
	...
	tt.setEntry(board, parseMove("g3h5", board), 1, MATE_SCORE - 8, TranspositionTable::EXACT, 5);
	// now the current node is 2 ply from root
